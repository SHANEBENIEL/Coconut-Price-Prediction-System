function generateDataset() {
  ...
  const sIdx=[7,5,2,-1,-4,-6,-5,-3,0,3,6,8]; // seasonal index by month
  const data=[];
  const st={};
...
/* ══════════════════════════════════════════
   2. AR(p) MODEL — OLS via Normal Equations
══════════════════════════════════════════ */
...
/* ══════════════════════════════════════════
   3. HOLT-WINTERS DOUBLE EXP SMOOTHING
══════════════════════════════════════════ */
...
function fitHW(p){
  ...
  for(let a=0.05;a<=0.95;a+=0.1) for(let b=0.05;b<=0.5;b+=0.1){
    const e=hwSSE(p,a,b);
...
/* ══════════════════════════════════════════
   4. LSTM — TensorFlow.js
══════════════════════════════════════════ */
const LB=30, EPOCHS=40;
...
async function trainLSTM(prices,onEpoch){
  ...
  m.add(tf.layers.lstm({units:32,returnSequences:true,inputShape:[LB,1],dropout:0.1}));
  m.add(tf.layers.lstm({units:16,returnSequences:false,dropout:0.1}));
  m.add(tf.layers.dense({units:1}));
  m.compile({optimizer:tf.train.adam(0.003),loss:'meanSquaredError'});
  await m.fit(xTr,yTr,{epochs:EPOCHS,batchSize:32,shuffle:true,
...
async function trainAll(){
  ...
  elog(`AR(7) β=[${arBeta.map(v=>v.toFixed(3)).join(', ')}]`);
  prog(28); await delay(60);
  ...
  step(2,'done',`MAPE ${hwMP}% · α=${hwP.alpha.toFixed(2)} β=${hwP.beta.toFixed(2)}`);
  elog(`HW α=${hwP.alpha.toFixed(2)}, β=${hwP.beta.toFixed(2)}, SSE=${hwP.sse?.toFixed(0)}`);
  prog(42); await delay(60);
  ...
  lstmObj=await trainLSTM(trP,(ep,loss)=>{
    lastL=loss;
    if(ep%8===0) elog(`LSTM epoch ${ep+1}/${EPOCHS} loss=${loss.toFixed(6)}`);
    prog(42+Math.round((ep/EPOCHS)*38));
  });
  const lmVal=await lstmForecast({...lstmObj,n:lstmObj.n.slice(0,TR-(LB))},vaP.length);
  // Use full norm for proper rollout
  const lmVal2=await lstmForecast(lstmObj,vaP.length);
  const lmMP=mape(lmVal2,vaP).toFixed(2);
  step(3,'done',`MAPE ${lmMP}% · final loss ${lastL.toFixed(5)}`);
  prog(84); await delay(60);
  ...
  elog(`Weights: AR=${WT.ar.toFixed(2)} HW=${WT.hw.toFixed(2)} LSTM=${WT.lm.toFixed(2)}`);
  elog(`Ens MAPE=${enMP}% MAE=₹${enMA} CI±₹${CI.q80.toFixed(1)}/₹${CI.q95.toFixed(1)}`);
  step(4,'done',`MAPE ${enMP}% · MAE ₹${enMA}`);
  prog(100); await delay(400);
...
function populateAll(){
  ...
  document.getElementById('sc0s').textContent=`${chg>=0?'▲':'▼'} ₹${Math.abs(chg).toFixed(1)} (${Math.abs(chgP).toFixed(1)}%) vs yesterday`;
  ...
  document.getElementById('sc3s').textContent=`MAE ₹${metrics.mae} · AR ${metrics.ar}% · LSTM ${metrics.lm}%`;
...
async function saveRec(){
  ...
  PP=DS.filter(d=>d.market==='Pollachi').map(d=>d.modalPrice);
  toast('♻ Retraining AR & HW, refreshing LSTM forecast…');
